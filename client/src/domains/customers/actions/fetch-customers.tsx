...
export const fetchCustomers =
    async (): Promise<ResponseFromCustomersFetchDto | void> => {
        const timer = startLoading();

        try {
            const { data }: { data: ResponseFromCustomersFetchDto } =
                await axios.get(APIRoutes.FETCH_CUSTOMERS, {
                    headers: {
            ...
            if (err.response.status === 403)
                setAppData({
                    mainInfo:
                        'Nie masz dpstępu do tych danych lub tej czynności!',
                    detailsArr: [],
                });
